...
# Adding human feedback mechanism to the RAG pipeline
...
class Crew:
    ...
    def process_query(self, query: str) -> str:
        ...
        # Collect human feedback after generating the final response
        feedback = self.agents["FeedbackCollector"].collect_feedback(final_response)
        
        return final_response, feedback
...
class FeedbackCollector:
    def collect_feedback(self, response: str) -> str:
        # Simulated human feedback mechanism
        print("\n--- Feedback Collection ---")
        print(f"Generated Response:\n{response}\n")
        feedback = input("Was this response helpful? (yes/no): ").strip().lower()
        
        if feedback == "yes":
            print("[FeedbackCollector] Positive feedback recorded.")
        elif feedback == "no":
            print("[FeedbackCollector] Negative feedback recorded.")
        else:
            print("[FeedbackCollector] Feedback unclear, no record.")
...
crew.add_agent(FeedbackCollector(), "FeedbackCollector")

# Test the pipeline with a sample query
query = "What are the latest advancements in AI ethics?"
final_response, feedback = crew.process_query(query)
print("\nFinal Response:\n", final_response)
